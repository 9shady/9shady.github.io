...
class CMB2_Type_File extends CMB2_Type_File_Base {
	...
	public function render( $args = array() ) {
		...
		$a = $this->args = $this->parse_args( 'file', array(
			...
			'type'            => array_key_exists( 'url', $options ) && false === $options['url'] ? 'hidden' : 'text',
		), $args );

		// get an array of image size meta data, fallback to 'large'
		$this->args['img_size_data'] = $img_size_data = parent::get_image_size_data( $a['preview_size'], 'large' );
		...
		$output .= sprintf( '<input class="cmb2-upload-button button-secondary" type="button" value="%1$s" />', esc_attr( $this->_text( 'add_upload_file_text', esc_html__( 'Add or Upload File', 'cmb2' ) ) ) );
