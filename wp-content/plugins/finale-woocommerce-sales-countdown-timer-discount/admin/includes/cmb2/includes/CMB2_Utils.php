...
class CMB2_Utils {
	...
	public static function make_valid_time_stamp( $string ) {
		...
		return self::is_valid_time_stamp( $string ) ? (int) $string : strtotime( (string) $string );
	}
	...
	public static function get_url_from_dir( $dir ) {
		...
		if ( 0 === strpos( $test_dir, self::normalize_path( WPMU_PLUGIN_DIR ) ) || 0 === strpos( $test_dir, self::normalize_path( WP_PLUGIN_DIR ) ) ) {
		...
		if ( 0 === strpos( $dir, $theme_root ) ) {
			...
			return set_url_scheme( trailingslashit( str_replace( untrailingslashit( $theme_root ), untrailingslashit( get_theme_root_uri() ), $dir ) ) );
		}
		...
		$url = str_replace( array( $site_dir, WP_PLUGIN_DIR ), array( $site_url, WP_PLUGIN_URL ), $dir );
