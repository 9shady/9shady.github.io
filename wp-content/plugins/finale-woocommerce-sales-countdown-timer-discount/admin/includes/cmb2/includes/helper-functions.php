...
function cmb2_get_oembed( $args = array() ) {
	...
	$error = sprintf( /* translators: 1: results for. 2: link to codex.wordpress.org/Embeds */
		esc_html__( 'No oEmbed Results Found for %1$s. View more info at %2$s.', 'cmb2' ), $oembed['fallback'], '<a href="https://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>' );
...
function cmb2_print_metabox_form( $meta_box, $object_id = 0, $args = array() ) {
	...
	// check permissions
	// @todo more hardening?
	if ( $cmb->prop( 'save_fields' ) // check nonce
	     && isset( $_POST['submit-cmb'], $_POST['object_id'], $_POST[ $cmb->nonce() ] ) && wp_verify_nonce( $_POST[ $cmb->nonce() ], $cmb->nonce() ) && $object_id && $_POST['object_id'] == $object_id ) {
...
if ( ! function_exists( 'date_create_from_format' ) ) {
	...
	function date_create_from_format( $date_format, $date_value ) {
		...
		$ymd = sprintf( /*
			 ...
			 */
			'%04d-%02d-%02d %02d:%02d:%02d', $parsed_time['tm_year'] + 1900,  // This will be "111", so we need to add 1900.
			$parsed_time['tm_mon'] + 1,      // This will be the month minus one, so we add one.
			$parsed_time['tm_mday'], $parsed_time['tm_hour'], $parsed_time['tm_min'], $parsed_time['tm_sec'] );
