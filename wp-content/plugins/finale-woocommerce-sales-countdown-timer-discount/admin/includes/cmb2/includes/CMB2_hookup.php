...
class CMB2_hookup extends CMB2_Hookup_Base {
	...
	public function term_hooks() {
		...
		foreach ( $this->taxonomies as $taxonomy ) {
			...
			$show_on_add = is_array( $show_on_term_add ) ? in_array( $taxonomy, $show_on_term_add ) : (bool) $show_on_term_add;
	...
	public function save_post( $post_id, $post = false ) {
		...
		$do_not_pass_go = ( ! $this->can_save( $post_type ) // check user editing permissions
		                    || ( 'page' == $post_type && ! current_user_can( 'edit_page', $post_id ) ) || ! current_user_can( 'edit_post', $post_id ) );
	...
	public function can_save( $type = '' ) {
		...
		$can_save = ( $this->cmb->prop( 'save_fields' ) // check nonce
		              && isset( $_POST[ $this->cmb->nonce() ] ) && wp_verify_nonce( $_POST[ $this->cmb->nonce() ], $this->cmb->nonce() ) // check if autosave
		              && ! ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) // get the metabox types & compare it to this type
		              && ( $type && in_array( $type, $this->cmb->box_types() ) ) // Don't do updates during a switch-to-blog instance.
	...
	protected function maybe_enqueue_column_display_styles() {
		global $pagenow;
		if ( $pagenow && $this->cmb->has_columns && $this->cmb->prop( 'cmb_styles' ) && in_array( $pagenow, array( 'edit.php', 'users.php', 'edit-comments.php', 'edit-tags.php' ), 1 ) ) {
