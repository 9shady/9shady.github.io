...
class CMB2_REST_Controller_Fields extends CMB2_REST_Controller_Boxes {
	...
	public function get_items( $request ) {
		...
		foreach ( $this->rest_box->cmb->prop( 'fields', array() ) as $field ) {
			...
			if ( $this->field && $this->get_item_permissions_check_filter() ) {
				$fields[ $field['id'] ] = $this->server->response_to_data( $this->prepare_field_response(), isset( $this->request['_embed'] ) );
			}
		}
	...
	public function modify_field_value( $activity ) {
		...
		$this->field = $this->rest_box->field_can_edit( $this->field ? $this->field : $this->request->get_param( 'field_id' ), true );
		...
		$this->field->args["value_{$activity}"] = (bool) 'deleted' === $activity ? $this->field->remove_data() : $this->field->save_field( $this->request['value'] );
	...
	protected function prepare_field_data( CMB2_Field $field ) {
		...
		$rendered = ( $cb = $field->maybe_callback( 'render_row_cb' ) ) // Ok, callback is good, let's run it.
			? $this->get_cb_results( $cb, $field->args(), $field ) : false;
