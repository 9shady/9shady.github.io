...
class CMB2_REST_Controller_Boxes extends CMB2_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( $boxes as $this->rest_box ) {
			if ( // Make sure this box can be read
				$this->rest_box->rest_read // And make sure current user can view this box.
				&& $this->get_item_permissions_check_filter( $this->request ) ) {
				$boxes_data[] = $this->server->response_to_data( $this->get_rest_box(), isset( $this->request['_embed'] ) );
			}
		}
