...
class CMB2_Sanitize {
	...
	public function default_sanitization() {
		...
		if ( function_exists( 'apply_filters_deprecated' ) ) {
			$override_value = apply_filters_deprecated( "cmb2_validate_{$this->field->type()}", array(
				null,
				$this->value,
				$this->field->object_id,
				$this->field->args(),
				$this
			), '2.0.0', "cmb2_sanitize_{$this->field->type()}" );
		} else {
	...
	public function text_date_timestamp() {
		...
		return is_array( $this->value ) ? array_map( array( $this->field, 'get_timestamp_from_value' ), $this->value ) : $this->field->get_timestamp_from_value( $this->value );
	}
	...
	public function _get_group_file_value_array( $id_key ) {
		...
		$id_val = isset( $alldata[ $base_id ][ $i ][ $id_key ] ) ? absint( $alldata[ $base_id ][ $i ][ $id_key ] ) : '';
