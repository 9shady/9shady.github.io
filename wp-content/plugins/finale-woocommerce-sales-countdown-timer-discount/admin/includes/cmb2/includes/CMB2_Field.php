...
class CMB2_Field extends CMB2_Base {
	...
	public function get_data( $field_id = '', $args = array() ) {
		...
		if ( 'cmb2_field_no_override_val' === $data ) {
			$data = 'options-page' === $a['type'] ? cmb2_options( $a['id'] )->get( $a['field_id'] ) : get_metadata( $a['type'], $a['id'], $a['field_id'], ( $a['single'] || $a['repeat'] ) );
		}

		if ( $this->group ) {

			$data = is_array( $data ) && isset( $data[ $this->group->index ][ $this->args( '_id' ) ] ) ? $data[ $this->group->index ][ $this->args( '_id' ) ] : false;
		}
	...
	public function save_field_from_data( array $data_to_save ) {
		...
		$meta_value = isset( $this->data_to_save[ $this->id( true ) ] ) ? $this->data_to_save[ $this->id( true ) ] : null;
	...
	public function _set_field_defaults( $args ) {
		...
		$args['has_supporting_data'] = in_array( $args['type'], array(
