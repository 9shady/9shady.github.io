...
class CMB2_Ajax {
	...
	public function get_oembed( $args ) {
		...
		return sprintf( '<p class="ui-state-error-text">%s</p>', sprintf( /* translators: 1: results for. 2: link to codex.wordpress.org/Embeds */
	...
	public function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( ! $this->hijack || ( $this->object_id != $object_id && 1987645321 !== $object_id ) // only want to hijack oembed meta values
