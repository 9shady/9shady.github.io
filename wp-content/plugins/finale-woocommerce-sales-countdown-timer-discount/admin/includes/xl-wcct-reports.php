...
class XL_WCCT_Reports {
	 ...
	 * get campaigns for the order and genrates markup
	 *
	 * @param Integer $order_id
	 *
	 * @return string Markup
	 */
	public function order_running_campaign_view( $order_id ) {
		...
		if ( ! empty( $campaigns ) ) {
			...
			foreach ( $campaigns as $campaign ) {
				...
				if ( $get_campaign && $campaign == $get_campaign->ID ) {
					$campaigns_data[ $campaign ] = get_the_title( $campaign );
				}
			}
		}
	...
	protected function generate_markup( $view_type, $data ) {
		...
		if ( $view_type === 'order_mb' ) {
            ...
            <ul style="list-style-type: disc;padding-left: 17px;">
				<?php foreach ( $data as $campaign_key => $campaign_info ) {

					printf( '<li>%s #<a href="%s">%s</a></li>', $campaign_info, get_edit_post_link( $campaign_key ), $campaign_key );
				}
				?>
            </ul>
			<?php
