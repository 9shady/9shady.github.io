...
class WCCT_Admin {
	...
	public function hooks() {
		...
		add_action( 'post_updated', array( $this, 'restrict_to_publish_when_campaign_is_dissabled' ), 10, 3 );
	...
	public function restrict_to_publish_when_campaign_is_dissabled( $post_ID, $post_after, $post_before ) {
		remove_action( 'post_updated', array( $this, 'restrict_to_publish_when_campaign_is_dissabled' ), 10 );
	...
	public function wcct_woocommerce_options_page() {
		if ( 'blank' == get_option( 'xl_is_opted', 'blank' ) ) {
            ...
            <div class="wrap cmb2-options-page wcct_global_option">
                ...
                <a href="<?php echo admin_url( 'post-new.php?post_type=' . WCCT_Common::get_campaign_post_type_slug() ) ?>"
                   class="page-title-action">Add New Campaign</a>
	...
	public function wcct_enqueue_admin_assets() {
		...
		if ( WCCT_Common::wcct_valid_admin_pages() ) {
			...
			wp_localize_script( 'wcct_admin-js', 'buy_pro_helper', array(
				...
				'call_to_action_text' => __( 'Upgrade To PRO &nbsp;<i class="dashicons dashicons-arrow-right-alt"></i>', 'finale-woocommerce-sales-countdown-timer-discount' ),
				'protabs'             => array(
				...
				'popups'              => array(
					'inventory_adv'          => array(
						'title'   => __( "Advanced inventory is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'inventory_range'        => array(
						'title'   => __( "Invenory Range is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'recurring'              => array(
						'title'   => __( "Recurring Campaign is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'#wcct_events_settings'  => array(
						'title'   => __( "Events is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'#wcct_actions_settings' => array(
						'title'   => __( "Actions is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'#wcct_misc_settings'    => array(
						'title'   => __( "Advanced Settings is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'sticky-header'          => array(
						'title'   => __( "Sticky Header is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'sticky-footer'          => array(
						'title'   => __( "Sticky Footer is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'custom-text'            => array(
						'title'   => __( "Custom Text is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'custom-css'             => array(
						'title'   => __( "Custom CSS is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
					...
					'#wcct_coupon_settings'  => array(
						'title'   => __( "Coupons is a PRO Feature", 'finale-woocommerce-sales-countdown-timer-discount' ),
	...
	public function wcct_edit_form_top() {
		...
		?>
        <div class="notice">
            <p><?php _e( 'Back to <a href="' . admin_url( 'admin.php?page=wc-settings&tab=' . WCCT_Common::get_wc_settings_tab_slug() . '' ) . '">' . WCCT_FULL_NAME . '</a> settings.', 'finale-woocommerce-sales-countdown-timer-discount' ); ?></p>
        </div>
		<?php
	}
		...
		if ( $post != null ) {
			if ( $post && $post->post_type == WCCT_Common::get_campaign_post_type_slug() ) {
				if ( isset( $_POST['_wcct_data_menu_order'] ) ) {
					wp_update_post( array( 'ID' => $post_id, 'post_name' => sanitize_title( $_POST['post_title'] ) . "_" . $post_id, 'menu_order' => $_POST['_wcct_data_menu_order'] ) );
				}
	...
	public function wcct_wp_print_scripts() {
		global $wp_scripts;
		$screen = get_current_screen();
		if ( is_object( $screen ) && isset( $screen->post_type ) && $screen->post_type == 'wcct_countdown' ) {
	...
	public function wcct_add_mergetag_text() {
		...
		if ( is_object( $screen ) && ( $screen->base == 'post' && $screen->post_type == WCCT_Common::get_campaign_post_type_slug() ) ) {
			?>
            ...
            <div style="display:none;" class="wcct_tb_content" id="wcct_inventory_out_of_stock_help">
                ...
                <p>Keep this setting to <strong><u>YES</u></strong> if you can fullfill the Out-of-Stock products.</p>
            </div>
			<?php
		}
	}
	...
	public function wcct_plugin_row_actions( $links, $file ) {
		if ( $file == WCCT_PLUGIN_BASENAME ) {
			...
			$links[] = '<a href="https://wordpress.org/support/view/plugin-reviews/finale-woocommerce-sales-countdown-timer-discount/" target="_blank"><span class="dashicons dashicons-thumbs-up"></span> Vote!</a>';
		}
	...
	public function maybe_show_advanced_update_notification() {
		...
		if ( is_object( $screen ) && ( 'plugins.php' == $screen->parent_file || 'index.php' == $screen->parent_file || 'xl-countdown-timer' == filter_input( INPUT_GET, 'tab' ) ) ) {
