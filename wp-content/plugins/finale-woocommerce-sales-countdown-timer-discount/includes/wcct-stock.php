...
class WCCT_stock {
	...
	public function __construct() {
		 ...
		 * In order to handle campaign inventory settings for running over our of stock products or not
			...
			if ( empty( $get_meta ) ) {
				...
				if ( $product->is_in_stock() && $product->is_type( 'simple' ) ) {
					...
					if ( is_singular( 'product' ) ) {
						$getall_variations = $product->get_available_variations();

						if ( $getall_variations && is_array( $getall_variations ) && count( $getall_variations ) > 0 ) {

							$all_variation_state = array();

							foreach ( $getall_variations as $variation ) {
