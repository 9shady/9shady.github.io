...
//        add_action('xl_licenses_right_content', array($this, 'wcct_add_right_section_on_xlpages'));
	...
	public function wcct_xl_expected_slug() {
		...
		if ( ! XL_admin_notifications::has_notification( 'license_needs_attention' ) && count( $invalid_licenses ) > 0 ) {
			$license_invalid_text = sprintf( __( '<p>You are <strong>not receiving</strong> Latest Updates, New Features, Security Updates &amp; Bug Fixes for <strong>%s</strong>. <a href="%s">Click Here To Fix This</a>.</p>', 'finale-woocommerce-sales-countdown-timer-discount' ), implode( ",", $invalid_licenses ), add_query_arg( array( 'tab' => 'licenses' ), $this->expected_url ) );
            ...
            <form method="post">
                ...
                <div class="xl_core_tools_button" style="margin-bottom: 10px;">
                    <a class="button button-primary button-large xl_core_tools_btn" data-plugin="nextmove-lite" href="<?php echo add_query_arg( array(
						...
						"download" => "true"
					), admin_url( "export.php" ) ) ?>"><?php echo __( "Export Finale Campaign", 'finale-woocommerce-sales-countdown-timer-discount' ) ?></a>
                    <button type="submit" class="button button-primary button-large xl_core_tools_btn" name="xl_tools_export_setting" value="finale-lite"><?php echo __( "Export Settings", 'finale-woocommerce-sales-countdown-timer-discount' ) ?></button>
                </div>
                <br>
            </form>
        </div>
