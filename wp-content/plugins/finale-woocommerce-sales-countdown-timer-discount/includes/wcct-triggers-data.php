		...
		if ( is_array( $contents ) && count( $contents ) > 0 ) {
			foreach ( $contents as $content_single ) {
				...
				if ( $skip_rules || WCCT_Common::match_groups( $content_id, $ID ) ) {
					$meta_query = apply_filters( 'wcct_product_meta_query', $wpdb->prepare( "SELECT meta_key,meta_value  FROM $wpdb->postmeta WHERE post_id = %d AND meta_key LIKE %s", $content_id, '%_wcct_%' ) );
					$cache_key  = 'wcct_countdown_meta_' . $content_id;
					...
					if ( false !== $cache_data ) {
						...
						if ( false !== $transient_data ) {
							...
							$get_product_wcct_meta = $wpdb->get_results( $meta_query, ARRAY_A );
			...
			foreach ( $meta_data as $campaign_id => $val ) {
				...
				if ( $start_date_timestamp > 0 && $end_date_timestamp > 0 && $todayDate > 0 ) {
					...
					if ( $todayDate >= $start_date_timestamp && $todayDate < $end_date_timestamp && ( $start_date_timestamp != $end_date_timestamp ) ) {
						...
						if ( isset( $val['deal_enable_price_discount'] ) && $val['deal_enable_price_discount'] == '1' && ( isset( $val['deal_amount'] ) && $val['deal_amount'] != '' ) ) {
						...
						if ( isset( $val['deal_enable_goal'] ) && $val['deal_enable_goal'] == '1' ) {
							$allow_backorder = 'no';
						...
						if ( $threshold_reach_out == true ) {
							...
							// countdown bar array prepration
