		...
		if ( $product && isset( $rule_data['condition'] ) && isset( $rule_data['operator'] ) ) {
			...
			if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
				...
				if ( $get_parent_ID && $get_parent_ID !== '' ) {
					...
					$get_wcml_obj     = woocommerce_wpml::instance();
					$get_translate_id = get_post( $get_wcml_obj->products->get_original_product_id( $prod_id ) );
					if ( is_object( $get_translate_id ) && $get_translate_id instanceof WP_Post ) {
						$prod_id = $get_translate_id->ID;
					}
				}
			}
...
class WCCT_Rule_Product_Category extends WCCT_Rule_Base {
	...
	public function is_match( $rule_data, $productID ) {
		...
		if ( $productID && isset( $rule_data['condition'] ) && isset( $rule_data['operator'] ) ) {
			$product_types = wp_get_post_terms( $productID, 'product_cat', array( 'fields' => 'ids' ) );
			$in            = count( array_intersect( $product_types, $rule_data['condition'] ) ) > 0;
			$result        = $rule_data['operator'] == 'in' ? $in : ! $in;
		}
	...
	public function is_match( $rule_data, $productID ) {
		...
		if ( $productID && isset( $rule_data['condition'] ) && isset( $rule_data['operator'] ) ) {
			...
			foreach ( $rule_data['condition'] as $condition ) {
				...
				$post_terms = wp_get_post_terms( $productID, $attribute_taxonomy_name, array( 'fields' => 'ids' ) );
...
class WCCT_Rule_Product_Tags extends WCCT_Rule_Base {
	...
	public function is_match( $rule_data, $productID ) {
		...
		if ( $productID && isset( $rule_data['condition'] ) && isset( $rule_data['operator'] ) ) {
			$product_types = wp_get_post_terms( $productID, 'product_tag', array( 'fields' => 'ids' ) );
			$in            = count( array_intersect( $product_types, $rule_data['condition'] ) ) > 0;
			$result        = $rule_data['operator'] == 'in' ? $in : ! $in;
		}
