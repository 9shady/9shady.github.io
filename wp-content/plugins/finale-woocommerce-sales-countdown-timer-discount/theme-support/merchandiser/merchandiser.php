...
if ( ! function_exists( 'wcct_theme_helper_merchandiser' ) ) {
	...
	function wcct_theme_helper_merchandiser() {
		...
		add_action( 'woocommerce_after_single_product_summary', array( $wcct_core, 'wcct_position_below_meta' ), 8 );

		add_action( 'woocommerce_after_single_product_summary_data_tabs', array( $wcct_core, 'wcct_position_above_tab_area' ), 8 );
		...
		add_action( 'woocommerce_after_single_product_summary_related_products', array( $wcct_core, 'wcct_position_below_related_products' ), 23 );
		add_action( 'woocommerce_after_single_product_summary_related_products', function () {
			echo '</div></div>';
		}, 24 );
	}
